    ...
    def children(self, node):
        ...
        for d in directions:
            ...
            if 0 <= new_i < 3 and 0 <= new_j < 3:
                ...
                new_board[new_i][new_j] = temp
                #make a node from the matrix
                child_node = Node(None, new_board, 0, 0)
...
#     start_node = Node.Node(None, puzzle, 0, 0) #initial node, root
#     priority_queue = [] #nodes sorted by cost?
#     repeat_states = dict() #tracks visited states
#     min_heap_esque_queue.heappush(priority_queue, start_node) #start_node pushed into priority_queue

#     num_expanded_nodes = 0
#     max_queue_size = 0
#     repeat_states[start_node.board_to_tuple()] = "PARENT BOARD"

#     stack_print = [] #boards stored in a stack

#     while len(priority_queue) > 0:
#         max_queue_size = max(len(priority_queue), max_queue_size)

#         #node from queue checked
#         node_from_queue = min_heap_esque_queue.heappop(priority_queue) #pop the first board in the queue
#         repeat_states[node_from_queue.board_to_tuple()] = "this can be anything"
...
def general_search(puzzle):
    ...
    repeat_states = dict()
    priority_queue = []
    curr_node= Node.Node(None, puzzle, 0, 0)
    priority_queue.append(curr_node) #push into queue

    num_expanded_nodes = 0
    max_queue_size = 0
    repeat_states[curr_node.board_to_tuple()] = "PARENT BOARD"
    ...
    while len(priority_queue):
        ...
        for c in children:
            if c.board_to_tuple() not in repeat_states:
                ...
                repeat_states[c.board_to_tuple()] = "EXPANDED"
    return "failure"
    #end
